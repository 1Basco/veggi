...
const Task = ({ task, setCurrentId, currentUserId }) => {
  ...
  return (
    <Card className={classes.card}>
      ...
      <CardActions>
        ...
        {/* Botão para mudar o state da task para true queria fazer aqui mas me perdi um pouco pra onde ir */}
        {/* <FormControlLabel 
          //disable if task.state is false
          disabled={task.state}
          control={<Checkbox
            onChange={() => {}}
            value={task.state}
            />} 
          label={task.state ? 'Finished' : 'ToDo'}
          checked={task.state} /> */}
      </CardActions>
    </Card>
  );
